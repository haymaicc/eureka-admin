...
public class HttpUtil {
	...
	public static HttpURLConnection getHttpConnection(String url, String method, Map<String, String> headers)
		...
		if (_url.getUserInfo() != null) {
			String basicAuth = "Basic " + new String(new Base64().encode(_url.getUserInfo().getBytes()));
	...
	/**
	 * 
	 * 发送 POST 请求 考虑添加一个参数 Map<String, String> queryParas： getHttpConnection(buildUrl(url, queryParas), POST, headers);
	 */
	...
	private static String send(String url, String method) {
		...
		try {
			...
			conn = getHttpConnection(buildUrlWithQueryString(url, null, CHARSET), method, headers);
			conn.connect();
			OutputStream out = conn.getOutputStream();
			out.flush();
			out.close();
